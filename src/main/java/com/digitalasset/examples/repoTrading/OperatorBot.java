...
public class OperatorBot extends RepoMarketBot {
  ...
  private void loadTradingParties() {
    tradingParties =
        getConfiguration().getTradingParties().values().stream()
  ...
  private void createGenesisContract() {
    // command that creates the initial Ping contract with the required parameters according to the
    // model
    CreateCommand createCommand =
        new CreateCommand(
            genesisTemplateId,
            new Record(
                genesisTemplateId, new Record.Field("operator", new Party(getOperatorName()))));
  ...
  public Stream<CommandsAndPendingSet> process(LedgerViewFlowable.LedgerView<Template> ledgerView) {
    ...
    return Stream.concat(
        ledgerView.getContracts(genesisTemplateId).entrySet().stream()
            .map(this::inviteCcpAndPaymentProcessor),
  ...
  private CommandsAndPendingSet inviteTradingParties(Map.Entry<String, Template> entry) {
    ...
    if (ccpContractId == null) {
      ...
      DamlList args =
          new DamlList(tradingParties.stream().map(Party::new).collect(Collectors.toList()));
      return newCommandAndPendingSet(
          ONBOARDING_WORKFLOW_ID,
          Collections.singletonList(
              newExercise(
                  ...
                  new Record.Field("tradingParties", args))),
          HashTreePMap.empty());
    } else {
      return CommandsAndPendingSet.empty;
    }
  ...
  public TransactionFilter getTransactionFilter() {
    InclusiveFilter inclusiveFilter =
        new InclusiveFilter(new HashSet<>(Arrays.asList(genesisTemplateId, ccpTemplateId)));
